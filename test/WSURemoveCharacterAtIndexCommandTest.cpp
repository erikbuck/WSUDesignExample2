...
TEST(RemoveCharacterAtIndexCommand, getStringRepresentation) {
    ...
    WSU::Model::RemoveCharacterAtIndexCommand cmd0 {storesString_p, 0 };
...
TEST(RemoveCharacterAtIndexCommand, makeCommandWithName) {
    ...
    auto cmd_p = WSU::Model::StoredString::makeCommandWithName(WSU::Model::RemoveCharacterAtIndexCommand::name, storesString_p, obj.dump());
    if(nullptr != cmd_p) {
        cmd_p->run();
    }
    EXPECT_EQ("#!", storesString_p->getString());
